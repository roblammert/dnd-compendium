...
## Player Architect (Experimental)

v0.33.0 introduces **Player Architect**, an isolated experimental character-generation workflow under Tools > Player. It uses the entire active compendium for early race/class/background selection and records automatic/manual character modifications in a live Character Blueprint rather than overwriting base ability values. See `PLAYER_ARCHITECT.md` for the Stage 1 architecture and scope.
