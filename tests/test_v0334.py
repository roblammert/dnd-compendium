...
def test_2014_monk_proficiencies_become_blueprint_and_choices():
    ...
    assert pairs["Weapon Proficiencies"] == "Simple weapons, shortswords"
    assert pairs["Armor Proficiencies"] == "None"
    entries = _auto_entries(monk, "Class")
    assert any(row["stat"] == "Weapon Proficiencies" and "Simple weapons" in row["modifier"] for row in entries)
    assert not any(row["stat"] == "Saving Throws" for row in entries)
    notes = _class_choice_notes(monk)
    assert any(row["stat"] == "Tool Proficiencies" and "Choose one" in row["instruction"] for row in notes)
    assert any(row["stat"] == "Skill Proficiencies" and "Choose two" in row["instruction"] for row in notes)
...
def test_blueprint_drawer_has_player_decision_section_and_extended_stats():
    ...
    for stat in ["Weapon Proficiencies", "Armor Proficiencies", "Tool Proficiencies", "Saving Throws", "Skill Proficiencies", "Cantrips", "Spells", "Feats"]:
